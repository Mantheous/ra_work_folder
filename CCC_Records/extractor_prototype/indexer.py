"""
Stage 1: Search & Index
Queries the NARA API, paginates through all results, and populates the
SQLite database with record NAIDs and their associated page ObjectIDs.

Usage:
    python indexer.py                             # Default query
    python indexer.py "Civilian Conservation Corps"
    python indexer.py '"Kings Mountain" AND "Co 4479"'
"""
...
def index_query(query: str):
    ...
    while True:
        ...
        for hit in hits:
            ...
            # Upsert into 'records'
            conn.execute(
                "INSERT OR IGNORE INTO records (na_id, title) VALUES (?, ?)",
                (na_id, title),
            )
            records_added += 1
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Index NARA records into SQLite.")
    parser.add_argument(
        "query",
        nargs="?",
