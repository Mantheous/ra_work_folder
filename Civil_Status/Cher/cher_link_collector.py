...
def jump_to_page(target_page: int):
    ...
    for i in range(2, target_page):
        nav_buttons = page.locator('//*[@id="cms_colonne_centre"]/div[2]/div/div[2]/div/div[1]/div[2]/nav/ul')
...
def get_link(row) -> str:
    ...
    # if i == 0 and page_number == 1: # click the terms and conditions for the first record
    #     click_terms()
...
def process_row(i: int):
    try:
        wait_for_load()
        table_body = page.locator('//*[@id="cms_colonne_centre"]/div[2]/div/div[2]/div/div[2]/table/tbody')
        row = table_body.locator('tr').nth(i)
        # Copy down some data from the table
        commune = row.locator("td").nth(0).inner_text()
        period = row.locator("td").nth(1).inner_text()
        cote = row.locator("td").nth(2).inner_text()
        act_types = row.locator("td").nth(3).inner_text()
        ...
        tries += 1
        if tries > max_tries:
            notify(message="Cher scraper failed to process row after max tries", subject="Cher Scraper Error")
            print("exiting...")
            exit()
...
def scrape_page(page_number: int, starting_row: int = 0):
    ...
    for i in range(starting_row, row_count): #range(1):  # for debugging only
...
if __name__ == "__main__":
    with sync_playwright() as pw:
        ...
        try:
            number_of_records = int(page.locator('//*[@id="cms_colonne_centre"]/div[2]/div/div[2]/div/div[1]/div[1]/span').inner_text().replace('\u202f', ''))
        except:
            number_of_records = 11254
        ...
        while(page_number <= number_of_records // results_per_page):
            try:
                ...
                tries += 1
                if tries > max_tries:             
                    notify(message=f"Cher scraper failed to process page {page_number} after max tries", subject="Cher Scraper Error")
                    print("exiting...")
                    exit()
                jump_to_page(page_number)
            
    notify(message="Cher scraper has finished", subject="Cher Finished")
